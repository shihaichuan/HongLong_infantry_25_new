...
std::vector<base_interfaces::msg::Armor>
ArmorPoseEstimator::extractArmorPoses(const std::vector<Armor> &armors,
                                   Eigen::Matrix3d R_imu_camera) {
  std::vector<base_interfaces::msg::Armor> armors_msg;
      // std::cout<<"111  "<< armors.size()<<std::endl;
...
void ArmorPoseEstimator::sortPnPResult(const Armor &armor,
 ...
 // std::cout<<"rpy1[2]" <<rpy1[2]<<std::endl;
  ...
  //   return;
  // }
  constexpr float ROLL_THRES_DEG = 10.0;

  if ((error2 / error1 > PROJECT_ERR_THRES) || 
    (abs(rpy1[0]) > ROLL_THRES_DEG * M_PI/180) ||
    (abs(rpy2[0]) > ROLL_THRES_DEG * M_PI/180)) {
  return;
  }
  ...
  double r_angle =
      std::atan2(armor.right_light.axis.y, armor.right_light.axis.x) * 180 / M_PI;
