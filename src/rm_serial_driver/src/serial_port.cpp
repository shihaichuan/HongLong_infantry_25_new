...
namespace rm_serial_driver
{
  ...
  bool Port::setPermission(std::string name)
  {
  ...
  int Port::openPort()
  {
    ...
    setPermission(config->devname);
    fd = open(config->devname.c_str(), O_RDWR | O_NOCTTY | O_NDELAY);

    if (fd < 0)
    {
      // perror("open device failed!");
      
      printf("open device failed:%s\n", config->devname.c_str());
      isopen = false;
      // 开始尝试打开其他串口
    ...
    if (isatty(fd) == 0)
    {
      ...
      std::cout << "not tty device." << std::endl;
      isopen = false;
    }
    else
    {
      // printf("tty device test ok.\n");
      std::cout << "tty device test ok" << std::endl;
      isopen = true;
      init();
    }
    return fd;
